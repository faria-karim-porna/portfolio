...
const ProjectsCard = (props) => {
    ...
    <p className = "project-detail">{project.project_description}</p>
                            {
                                technologies.map(technology => <Technologies technology = {technology}></Technologies>)
                            }
                            <div className = "project-buttons">{ github.map(github => <Github github = {github}></Github>)} <button>Live Site</button></div>
                        </div>
                    </div>
                </div>
    );
